...
def run():
    ...
    img = Image.open('./Logo/ai_Logo.jpg')
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                # video_id = resume_vid.split("/")[-1]
                # res_vid_title = fetch_yt_video(video_id)
                ...
                st.header("**Bonus Video for Interview👨‍💼 Tips💡**")
                interview_vid = random.choice(interview_videos)
                video_id = interview_vid.split("/")[-1]
                int_vid_title = fetch_yt_video(video_id)
                # st.subheader("✅ **" + int_vid_title + "**")
                st.video(f"https://www.youtube.com/watch?v={video_id}")
        ...
        if st.button('Login'):
            if ad_user == 'bharat_bhai' and ad_password == 'bharat123':
                st.success("Welcome Bharat")
                # Display Data
                ...
                labels = plot_data.Predicted_Field.unique()
                print( labels)
                values = plot_data.Predicted_Field.value_counts().values.tolist()  # Convert Series to list
                print( values)
                st.subheader("📈 **Pie-Chart for Predicted Field Recommendations**")
                ...
                labels = plot_data.User_level.unique()
                print( labels)
                values = plot_data.User_level.value_counts().values.tolist()  # Convert Series to list
                print( values)
                st.subheader("📈 **Pie-Chart for User's👨‍💻 Experienced Level**")
...
def fetch_yt_video(video_id):
    ...
    try:
        ...
        print(f"Failed to fetch video information for {url}: {e}")
